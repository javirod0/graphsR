...
keys <- spotifyOAuth("spotygraph", "77b8cff16f7846fc839cf587542ccc29", "f32a3d56af5044d699c62db3dcbc1156")
user <- getUser("22k5uejuzzdeondos267t6sfy", token = keys)
...
paises_es <- c("Argentina", "Bolivia", "Chile", "Colombia", "Costa Rica",
               ...
               "Paraguay", "Perú", "Puerto Rico", "Uruguay")

tops_50 <- getPlaylists("22k5uejuzzdeondos267t6sfy", token = keys)
...
viralcharts_user = "22k5uejuzzdeondos267t6sfy"

canciones_tops50_es <- purrr::map(tops_50_es$id[-length(tops_50_es$id)],
                                  ~ getPlaylistSongs(user_id = viralcharts_user,
                                                     .x,
                                                     token = keys))
canciones_tops50_es[[18]] <- getPlaylistSongs(user_id = "22k5uejuzzdeondos267t6sfy",
